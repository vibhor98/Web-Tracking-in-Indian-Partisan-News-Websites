...
This module is used to extract persistent cookie IDs using a two crawl method
...
def extract_persistent_ids_from_dbs(cookie_dbs, visit_id, num_days=30):
    for cookie_db in cookie_dbs:
        ...
        for domain, name, value, access, expiry \
                in curr.execute('SELECT host, name, value, time_stamp, expiry FROM javascript_cookies'+' WHERE visit_id = '+str(visit_id)):
            ...
            if (((expiry - access).total_seconds())/60/60/24) < num_days:
                continue
...
# 5. the ids remain consistent through a crawl
# returns a dictionary with domains as keys and the various cookie names as a value list
# INPUT must be of the form: [profile_1, profile_2] 
def extract_common_id_cookies(cookie_id_dicts, short_cutoff=11, long_cutoff=100, sim=.33):
...
def extract_known_cookies_from_db(db_name, cookie_id_dict, visit_id):
    conn = lite.connect(db_name)
    cur = conn.cursor()

    found_cookies = {}
    for domain, name, value in cur.execute('SELECT host, name, value FROM javascript_cookies'+' WHERE visit_id = '+str(visit_id)):
...
def map_list_of_ids_to_cookies(known_cookies):
    id_dict = defaultdict(list)

    for cookie in known_cookies:
        for id in known_cookies[cookie]:
            id_dict[id].append(cookie)
